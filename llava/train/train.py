...
class ModelArguments:
    encoder_version: str = field(default=None) #v1 has the text encoder, v2 has the learnable tokens, v3 combination of two
    num_learnable_tokens: int = field(default=None)
    mm_text_select_layer: int = field(default=None) 
    mm_text_select_feature: Optional[str] = field(default="cls_patch")
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        torch.save(trainer.model.get_vision_tower().state_dict(), 
                   os.path.join(output_dir, f'vision_tower.bin'))
        print('if getattr(trainer.args')
        # Only save Adapter
    ...
    if trainer.deepspeed:
        print('trainer.deepspeed')
        torch.cuda.synchronize()
        trainer.save_model(output_dir)
        keys_to_match = list()    
        if hasattr(trainer.model.get_model().get_vision_tower(), 'text_projection'):
            keys_to_match.extend(['text_projection', 'image_text_infusion'])
        if hasattr(trainer.model.get_model().get_vision_tower(), 'learnable_tokens'):
            keys_to_match.append('learnable_tokens')
            
        torch.save(get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match),
               os.path.join(output_dir, f'vision_tower.bin'))
        return
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.tune_mm_mlp_adapter:
            ...
            if hasattr(model.get_model().get_vision_tower(), 'learnable_tokens'):
                model.get_model().get_vision_tower().learnable_tokens.requires_grad = True
    ...
    data_module = make_supervised_data_module(prompt_tokenizer=vision_tower.tokenizer,
                                              tokenizer=tokenizer,
                                              data_args=data_args)
    ...
    torch.save(vision_tower.state_dict(), os.path.join(training_args.output_dir, 
                                                              f'vision_tower.bin'))
    trainer.save_state()
...
def preprocess_vision_prompt(sources, prompt_tokenizer):
    ...
    for source in sources:
        if source['from'] == 'human':
            ...
            if ':' in source['value']:
                source['value'] = source['value'].split(':')[1].lstrip()
    ...
    # all_prompts = prompt_tokenizer(all_prompts)
    # return all_prompts[random.randint(0, len(all_prompts)-1)].unsqueeze(0)
