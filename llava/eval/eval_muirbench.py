...
def eval_model(args):
    ...
    for row in tqdm(ds['test']):
        ...
        img_idx = 0
        for idx, token in enumerate(question):
            if '<image>' in token and img_idx == 0:
                # question[idx] = question[idx].replace('<image>', '<image> Left Image')
                img_idx += 1
                
            elif '<image>' in token and img_idx != 0:
                question[idx] = question[idx].replace('<image>', 'Right Image')
        ...
        image_tensor = process_images([new_image], image_processor, model.config)[0]
        
        with torch.inference_mode():
            output_ids = model.generate(
                inputs=input_ids,
                images=image_tensor.unsqueeze(0).half().cuda(),
                image_sizes=[new_image.size],
